...
Name: dcc-mcp-unreal
Version: 0.3.0
Requires: dcc-mcp-core>=0.20.13,<1.0.0
```
...
| `DCC_MCP_CORE_SPEC` | `dcc-mcp-core>=0.20.13,<1.0.0` | Core version spec (for plugin builds) |
...
`dcc-mcp-unreal` pins `dcc-mcp-core>=0.20.13,<1.0.0`. If a newer core
broke compatibility, pin the known-good version:

```bash
"C:\Program Files\Epic Games\UE_5.7\Engine\Binaries\ThirdParty\Python3\Win64\python.exe" -m pip install "dcc-mcp-core>=0.20.13,<1.0.0"
```
